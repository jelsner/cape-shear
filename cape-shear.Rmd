...
BigDays.sfdfT <- BigDays.sfdfT %>%
  mutate(A = as.numeric(HullArea)/10^10, #convert square kilometers
         CAPE = maxCAPE/1000,
         CIN = minCIN/100,
         DLBS = maxBS_deep/10,
...
Create a time series of average tornadoes per cluster by year. 
```{r}
tornyear <- BigDays.sfdfT %>%
  group_by(Year) %>%
  summarize(AvgTornperYear = mean(nT))
```

```{r}
TornYear <- ggplot(tornyear, (aes(Year, y = AvgTornperYear))) +
  geom_smooth(method = lm, se = FALSE, size = 1, col = "blue") +
  geom_line(color = "black", lwd = 1) +
  scale_x_continuous(expand = c(0, 0), breaks = c(seq(1994,2018,1)), limits = c(1994, 2018)) +
  scale_y_continuous(limits = c(10, 40), sec.axis = sec_axis(~.)) + 
#  geom_smooth(aes(x = week, y = probs/100), span = .5, se = FALSE, color = "gray70", size = 1) +
  theme_bw() +
  xlab("") +
  ylab("Average Tornadoes per Cluster\n ")

TornYear <- TornYear + theme(axis.title.y = element_text(colour = "black"), axis.text.y = element_text(color = "black"),  axis.text.x = element_text(angle = 45, hjust = 1), axis.text = element_text(size = 12), axis.title=element_text(size=14)) + ggtitle("Time Series")
TornYear
```
...
Break them into terciles for a random sample of 99 cases. 
```{r}
obs <- df[sample(nrow(df), 99), ]
pred <- obs$Predicted
obs <- obs$Observed
table(ntile(obs, 3), ntile(pred, 3))
```
...
df <- data.frame(ID = BigDays.sfdfT$ID,
                 ...
                 Area_sqkm = BigDays.sfdfT$A,
                 CAPE = BigDays.sfdfT$CAPE,
                 DLBS = BigDays.sfdfT$DLBS,
                 SLBS = BigDays.sfdfT$SLBS, 
                 nT = BigDays.sfdfT$nT)

df <- df%>%
  mutate(residuals = Observed - Predicted,
         clusdens = Observed/Area_sqkm) 
...
formulaT <- nT ~ Area_sqkm + CAPE + DLBS + SLBS
modelFinalT2 <- glm.nb(formulaT, data = Model_lessdens)
summary(modelFinalT2)

df <- Model_lessdens %>%
  mutate(Predicted = predict(modelFinalT2, type = "response"))

#Mean Absolute Error: 
mean(abs(df$nT - df$Predicted))

library(Metrics)
mae(df$nT, df$Predicted)
...
predover1.df <- df %>% filter(ID == 201105234655)
predover1.df
...
tm_shape(clusover1,projection = merc) + 
  ...
  tm_shape(clusover1torns, 
    ...
    tm_symbols(size = 3, 
               ...
               labels = c("0", "1", "2", "3", "4", "5"), 
               title.col = "EF Rating") +
    tm_layout(title = "May 23, 2011  (Overpredict) \n 17 tornadoes", 
              title.position = c("center", "top"), 
              legend.title.size = 1.4,
              legend.position = c("right", "bottom"), 
              legend.stack = "horizontal",
...
```{r}
#Extract the big day using the unique ID created. 
clusover2 <- BigDays.sfdfT %>%
  filter(ID == 200805253953)
...
predover2.df <- df %>% filter(ID == 200805253953)
predover2.df
...
tm_shape(clusover2,projection = merc) + 
  ...
  tm_shape(clusover2torns, 
    ...
    tm_symbols(size = 3, 
               ...
               labels = c("0", "1", "2", "3", "4", "5"), 
               title.col = "EF Rating") +
    tm_layout(title = "May 25, 2008  (Overpredict) \n 32 tornadoes", 
              title.position = c("center", "top"), 
              legend.title.size = 1.4,
              legend.position = c("right", "bottom"), 
              legend.stack = "horizontal",
...
predunder1.df <- df %>% filter(ID == 199901211532)
predunder1.df
...
tm_shape(clusunder1,projection = merc) + 
  ...
  tm_shape(clusunder1torns, 
    ...
    tm_symbols(size = 3, 
               ...
               labels = c("0", "1", "2", "3", "4", "5"), 
               title.col = "EF Rating") +
    tm_layout(title = "January 21, 1999  (Underpredict) \n 99 tornadoes", 
              title.position = c("center", "top"), 
              legend.title.size = 1.4,
              legend.position = c("right", "bottom"), 
              legend.stack = "horizontal",
...
predunder2.df <- df %>% filter(ID == 201104274630)
predunder2.df
...
tm_shape(clusunder2,projection = merc) + 
  ...
  tm_shape(clusunder2torns, 
    ...
    tm_symbols(size = 3, 
               ...
               labels = c("0", "1", "2", "3", "4", "5"), 
               title.col = "EF Rating") +
    tm_layout(title = "April 27, 2011 (Underpredict) \n 173 tornadoes", 
              title.position = c("center", "top"), 
              legend.title.size = 1.4,
              legend.position = c("right", "bottom"), 
              legend.stack = "horizontal",
...
Create a 4-panel map of the two  most over predicted and two most under predicted variables. 
```{r}
tmap_arrange(Over1, Over2, Under1, Under2)
```

Compare the correlation between Residuals and the Cluster Density: 
```{r}
cor(df$residuals, df$clusdens)
#0.02132019
```
...
day1out <- read_sf(dsn = "day1otlk_20200412_1200-shp",
...
mycols <- colors()[c(51, 142, 585, 555)]
...
tm_shape(Apr12.sfdfT) +
  tm_bubbles(size = 0.05, col = "black", border.lwd = 0, alpha = 0.6 ) +
  tm_layout(#title = "April 12, 2020 1200 UTC \n Day 1 Convective Outlook", 
              title.position = c("center", "top"), 
