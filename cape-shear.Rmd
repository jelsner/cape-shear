...
(2) Successively remove additional variables with Pr(>|t|) > .01

```{r}
df <- BigDays.sfdfT %>%
  st_drop_geometry() %>%
  select(nT, HullArea = as.numeric(HullArea), maxCAPE, maxDEW, maxHLCY, minCIN, maxUSTM, maxVSTM, maxBS_deep, maxBS_shallow, totalPOP, Lon, Lat)
```
