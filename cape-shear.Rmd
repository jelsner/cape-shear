...
(p1 <- ggplot(plot.predictions, aes(x = DLBS, y = probability, color = factor(P * 100000, levels = rev(Plevels * 100000)))) +
  geom_line(size = 2) +
...
( p2 <- ggplot(plot.predictions, aes(x = CAPE, y = probability, color = factor(P * 100000, levels = rev(Plevels * 100000)))) +
  geom_line(size = 2) +
  scale_y_continuous(limits = c(0, .25), labels = scales::percent) +
   scale_colour_manual(name = "Population", labels = c("8M", "4M", "1M"), values = cbPalette) + 
  ylab(paste("Probability of at least", nCthresh, "casualties")) +
