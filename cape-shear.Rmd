...
Protocol
(1) Use `step()` with `direction = "both"`
...
df <- BigDays.sfdfT %>%
  st_drop_geometry() %>%
  select(nT, maxCAPE, maxDEW, maxHLCY, minCIN, maxUSTM, maxVSTM, maxBS_deep, maxBS_shallow, totalPOP, Lon, Lat)

step(lm(log(nT) ~ ., data = df), direction = "both")

summary(lm(log(nT) ~  totalPOP + maxCAPE + maxVSTM + minCIN  + maxBS_deep + maxHLCY + Lat, data = BigDays.sfdfT))
summary(lm(log(nT) ~  totalPOP + maxCAPE + maxVSTM + minCIN  + maxBS_deep + maxHLCY, data = BigDays.sfdfT))

ranef(lmer(log(nT) ~ totalPOP + maxCAPE + maxBS_deep + maxVSTM + minCIN + maxHLCY  + (1|Mo), data = BigDays.sfdfT))

summary(lm(log(nT) ~  totalPOP + maxCAPE * maxBS_deep + maxVSTM + minCIN + maxHLCY, data = BigDays.sfdfT))

modelFinal <- lm(log(nT) ~  totalPOP + maxCAPE * maxBS_deep + maxVSTM + minCIN + maxHLCY, data = BigDays.sfdfT)
hist(resid(modelFinal))

modelFinal2 <- glm(I(nT - 10) ~  totalPOP + maxCAPE * maxBS_deep + maxVSTM + minCIN + maxHLCY, family = "poisson", data = BigDays.sfdfT)
...
# .6 in-sample, also check out of sample
```
